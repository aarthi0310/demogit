...
function selectPlan(planName, price, data, validity) {
    const phoneNumber = localStorage.getItem('phoneNumber') || 'defaultPhone'; // Ensure this is set during login
    const url = `/customer/payment.html?planName=${encodeURIComponent(planName)}&price=${encodeURIComponent(price)}&data=${encodeURIComponent(data)}&validity=${encodeURIComponent(validity)}&phoneNumber=${encodeURIComponent(phoneNumber)}`;
